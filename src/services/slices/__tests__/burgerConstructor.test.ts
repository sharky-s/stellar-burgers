...
const bun = { _id: 'bun1', name: 'Булка', type: 'bun', price: 100, image: '', image_large: '', image_mobile: '' } as const;
const main1 = { _id: 'main1', name: 'Котлета', type: 'main', price: 200, image: '', image_large: '', image_mobile: '' } as const;
const main2 = { _id: 'main2', name: 'Сыр', type: 'main', price: 50, image: '', image_large: '', image_mobile: '' } as const;

describe('burgerConstructor reducer', () => {
  it('добавляет булку', () => {
    const state = reducer(undefined, burgerConstructorActions.addIngredient(bun as any));
    expect(state.bun?._id).toBe('bun1');
    expect(state.ingredients).toHaveLength(0);
  });

  it('добавляет начинку', () => {
    let state = reducer(undefined, burgerConstructorActions.addIngredient(bun as any));
    state = reducer(state, burgerConstructorActions.addIngredient(main1 as any));
    expect(state.ingredients).toHaveLength(1);
    expect(state.ingredients[0]._id).toBe('main1');
  });

  it('удаляет ингредиент по id', () => {
    let state = reducer(undefined, burgerConstructorActions.addIngredient(main1 as any));
    state = reducer(state, burgerConstructorActions.addIngredient(main2 as any));
